...
function cRP.getSalary()
	...
	if user_id then
		...
		if vRP.hasPermission(user_id,"Police") then
			vRP.addBank(user_id,1235)
			TriggerClientEvent("Notify",source,"azul","Salário de <b>$1.235</b> recebido.",5000)
		end

		if vRP.hasPermission(user_id,"Paramedic") then
			vRP.addBank(user_id,1475)
			TriggerClientEvent("Notify",source,"azul","Salário de <b>$1.475</b> recebido.",5000)
		end

		if vRP.hasPermission(user_id,"Mechanic") then
			vRP.addBank(user_id,1085)
			TriggerClientEvent("Notify",source,"azul","Salário de <b>$1.085</b> recebido.",5000)
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if user_id and args[1] and vRP.getHealth(source) > 101 then
			if vRP.hasPermission(user_id,"Police") then
				...
				for k,v in pairs(policeResult) do
					async(function()
						TriggerClientEvent("chatME",v,"^2 911^3 POLICE^9"..identity["name"].."^0"..rawCommand:sub(4))
					end)
				end
			end
		end
		...
		if user_id and args[1] and vRP.getHealth(source) > 101 then
			if vRP.hasPermission(user_id,"Paramedic") then
				...
				for k,v in pairs(paramedicResult) do
					async(function()
						TriggerClientEvent("chatME",v,"^2 112^3 PARAMEDIC^9"..identity["name"].."^0"..rawCommand:sub(4))
					end)
				end
			end
		end
...
AddEventHandler("player:servicePolice",function()
	...
	if user_id then
		...
		elseif vRP.hasPermission(user_id,"waitPolice") then
			...
			TriggerEvent("blipsystem:serviceEnter",source,"Police",38)
...
AddEventHandler("player:serviceParamedic",function()
	...
	if user_id then
		...
		elseif vRP.hasPermission(user_id,"waitParamedic") then
			...
			TriggerEvent("blipsystem:serviceEnter",source,"Paramedic",35)
			TriggerClientEvent("Notify",source,"azul","Entrou em serviço.",5000)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYER:SERVICEMECHANIC
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("player:serviceMechanic")
AddEventHandler("player:serviceMechanic",function()
	...
	if user_id then
		if vRP.hasPermission(user_id,"Mechanic") then
			...
			TriggerEvent("blipsystem:serviceExit",source)
			vRP.updatePermission(user_id,"Mechanic","waitMechanic")
			TriggerClientEvent("paramedic:updateService",source,false)
			TriggerClientEvent("Notify",source,"azul","Saiu de serviço.",5000)
		elseif vRP.hasPermission(user_id,"waitMechanic") then
			vRP.insertPermission(source,"Mechanic")
			vRP.updatePermission(user_id,"waitMechanic","Mechanic")
			TriggerClientEvent("paramedic:updateService",source,true)
			TriggerEvent("blipsystem:serviceEnter",source,"Mechanic",60)
			TriggerClientEvent("Notify",source,"azul","Entrou em serviço.",5000)
		end
	end
end)
...
AddEventHandler("player:outfitFunctions",function(mode)
	...
	if user_id and not vRP.reposeReturn(user_id) and not vRP.wantedReturn(user_id) then
		...
		elseif mode == "Glasses" then
			TriggerClientEvent("skinshop:setGlasses",source)
		elseif mode == "Jacket" then
			TriggerClientEvent("skinshop:setJacket",source)
		elseif mode == "Shirt" then
			TriggerClientEvent("skinshop:setShirt",source)
		elseif mode == "Arms" then
			TriggerClientEvent("skinshop:setArms",source)
		elseif mode == "Pants" then
			TriggerClientEvent("skinshop:setPants",source)
		elseif mode == "Shoes" then
			TriggerClientEvent("skinshop:setShoes",source)
		elseif mode == "Vest" then
			TriggerClientEvent("skinshop:setVest",source)
		elseif mode == "Accessory" then
			TriggerClientEvent("skinshop:setAccessory",source)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand("me",function(source,args,rawCommand)
	if exports["chat"]:statusChat(source) then
		...
		if user_id and args[1] then
			local identity = vRP.userIdentity(user_id)
			if identity then
				TriggerClientEvent("showme:pressMe",-1,source,rawCommand:sub(3),6)

				local players = vRPC.nearestPlayers(source,5)
				for _,v in pairs(players) do
					TriggerClientEvent("showme:pressMe",v[2],source,rawCommand:sub(3),6)
				end
			end
		end
	end
