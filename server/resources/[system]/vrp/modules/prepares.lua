...
vRP.prepare("characters/getLocate","SELECT id FROM summerz_characters WHERE locate = @locate")
...
vRP.prepare("characters/setBlood","UPDATE summerz_characters SET blood = @blood WHERE id = @user_id")
...
vRP.prepare("characters/updateLocate","UPDATE summerz_characters SET locate = @locate WHERE id = @user_id")
...
vRP.prepare("characters/createCharacters","INSERT INTO summerz_characters(steam,name,name2,locate,phone) VALUES(@steam,@name,@name2,@locate,@phone)")
-----------------------------------------------------------------------------------------------------------------------------------------
-- ACCOUNTS
-----------------------------------------------------------------------------------------------------------------------------------------
...
vRP.prepare("accounts/infosUpdatePremiumchars","UPDATE summerz_accounts SET chars = chars + 2 WHERE steam = @steam")
...
vRP.prepare("vehicles/updateVehicles","UPDATE summerz_vehicles SET engine = @engine, body = @body, doors = @doors, windows = @windows, tyres = @tyres WHERE user_id = @user_id AND vehicle = @vehicle")
vRP.prepare("vehicles/updateNitro","UPDATE summerz_vehicles SET nitro = @nitro WHERE plate = @plate")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PROPERTYS
-----------------------------------------------------------------------------------------------------------------------------------------
...
vRP.prepare("propertys/updateVault","UPDATE summerz_propertys SET vault = vault + 10 WHERE name = @name AND owner = 1")
...
-- RACES
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("races/racesWin","SELECT raceid,user_id FROM summerz_races WHERE raceid = @raceid and user_id = @user_id")
vRP.prepare("races/updateRacesWin","UPDATE summerz_races SET name = @name, vehicle = @vehicle, points = @points, date = @date WHERE raceid = @raceid and user_id = @user_id")
vRP.prepare("races/insertRacesWin","INSERT INTO summerz_races(raceid,user_id,name,vehicle,points,date) VALUES(@raceid,@user_id,@name,@vehicle,@points,@date)")
vRP.prepare("races/showRacesWin","SELECT * FROM summerz_races WHERE raceid = @raceid ORDER BY points DESC LIMIT 5")