...
Citizen.CreateThread(function()
	...
	for k,v in pairs(vehicles) do
		if v[4] == "cars" then
			table.insert(typeCars,{ k = k, name = v[1], price = v[3], chest = parseInt(v[2]), tax = parseInt((v[3])*(v[6])) })
		elseif v[4] == "bikes" then
			table.insert(typeBikes,{ k = k, name = v[1], price = v[3], chest = parseInt(v[2]), tax = parseInt((v[3])*(v[6])) })
		elseif v[4] == "work" then
			table.insert(typeWorks,{ k = k, name = v[1], price = v[3], chest = parseInt(v[2]), tax = parseInt((v[3])*(v[6])) })
		elseif v[4] == "rental" then
			table.insert(typeRental,{ k = k, name = v[1], price = v[5], chest = parseInt(v[2]), tax = parseInt((v[3])*(v[6])) })
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
function cRP.requestPossuidos()
	...
	if user_id then
		...
		for k,v in pairs(vehicles) do
			...
			if parseInt(os.time()) <= parseInt(v["rental"] + 24 * v["rendays"] * 60 * 60) then
				vehicleRental = minimalTimers(parseInt(86400 * v["rendays"] - (os.time() - v["rental"])))
			end

			table.insert(vehList,{ k = v["vehicle"], name = vehicleName(v["vehicle"]), price = parseInt(vehiclePrice(v["vehicle"]) * 0.7), chest = vehicleChest(v["vehicle"]), tax = vehicleTax, plate = v["plate"], rental = vehicleRental })
		end
	...
	if user_id then
		...
		if vehicle[1] then
			if os.time() >= parseInt(vehicle[1]["tax"] + 24 * 7 * 60 * 60) then
				local vehiclePrice = parseInt(vehiclePrice(vehName) * vehicleTax(vehName))

				if vRP.request(source,"Pagar taxa de <b>$"..parseFormat(vehiclePrice).."</b> dólares?",30) then
...
function cRP.requestRental(vehName)
	...
	if user_id then
		if actived[user_id] == nil then
			...
			if vRP.request(source,"Alugar o veículo <b>"..vehicleName(vehName).."</b> pagando <b>"..parseFormat(vehPrice).." Gemas</b>?",30) then
...
function cRP.rentalMoney(vehName)
	...
	if user_id then
		if actived[user_id] == nil then
			...
			if vRP.request(source,"Alugar o veículo <b>"..vehicleName(vehName).."</b> pagando <b>$"..parseFormat(vehPrice).."</b>?",30) then
	...
	if user_id then
		if actived[user_id] == nil then
			...
			if not vRP.wantedReturn(user_id) then
				local request = vRP.request(source,"Deseja iniciar o teste pagando <b>$100</b>?",60)
				if request then
