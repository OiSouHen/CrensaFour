...
const updateDrag = () => {
	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			var contents = `<item>${name}</item>${description !== "undefined" ? "<br><description>"+description+"</description>":""}<br><legenda>Economia: <r>$${economy}</r> <s>|</s> Máximo: <r>${max !== "undefined" ? max:"S/L"}</r></legenda>`;
			...
			if (name == "Passaporte" || name == "Distintivo"){
				...
				contents = `<item>${name}</item>${description !== "undefined" ? "<br><description>"+description+"</description>":""}<br><legenda>Nome: <r>${idName}</r> <s>|</s> Porte: <r>${idPort == 0 ? "Não":"Sim"}</r><br>Tipo Sangüineo: <r>${idBlood}</r> <s>|</s> Nacionalidade: <r>${idNacionalidade}</r>${idGems !== "undefined" ? "<br>Gemas: <r>"+ formatarNumero(idGems) +"</r> <s>|</s> Premium: <r>"+ idPremium +"</r><br>Rolepass: <r>"+ idRolepass +"</r>":""}</legenda>`;
			}
