...
AddEventHandler("towdriver:Tow",function()
	if inService then
		local ped = PlayerPedId()
		local coords = GetEntityCoords(ped)
		local distance = #(coords - vector3(despawnLocates[serviceLocate][1],despawnLocates[serviceLocate][2],despawnLocates[serviceLocate][3]))
		if distance <= 30 then
			if not IsPedInAnyVehicle(ped) and GetGameTimer() >= timeSeconds and distance <= 5 and vehTower == nil then
