...
local robberys = {
	...
	["43"] = {
		["coords"] = { -821.74,-183.3,37.56 },
		["name"] = "Barbearia dos Bloods",
		["type"] = "barbershop",
		["distance"] = 10.0,
		["cooldown"] = 60,
		["item"] = "card04",
	...
	["44"] = {
		["coords"] = { 1211.53,-470.69,66.2 },
		["name"] = "Barbearia dos Families",
		["type"] = "barbershop",
		["distance"] = 10.0,
		["cooldown"] = 60,
		["item"] = "card04",
...
function cRP.checkRobbery(robberyId)
	...
	if user_id then
		if robberys[robberyId] then
			...
			if GetGameTimer() >= robberyAvailable[prev["type"]] and prev["police"] == 1 then
				...
				if parseInt(#policeResult) > parseInt(prev["cops"]) then
					...
					if vRP.tryGetInventoryItem(user_id,consultItem[2],1) then
						...
						local weazelResult = vRP.numPermission("Weazel")
						for k,v in pairs(weazelResult) do
							async(function()
								TriggerClientEvent("NotifyPush",v,{ code = "31", title = prev["name"], x = prev["coords"][1], y = prev["coords"][2], z = prev["coords"][3], time = "Recebido às "..os.date("%H:%M"), blipColor = 22 })
								vRPC.playSound(v,"Beep_Green","DLC_HEIST_HACKING_SNAKE_SOUNDS")
							end)
			...
			elseif GetGameTimer() >= robberyAvailable[prev["type"]] and prev["police"] == 0 then
				local gangResult = vRP.numPermission(prev["gang"])
				if parseInt(#gangResult) > parseInt(prev["members"]) then
					local consultItem = vRP.getInventoryItemAmount(user_id,prev["item"])
					if consultItem[1] <= 0 then
						TriggerClientEvent("Notify",source,"amarelo","Precisa de <b>1x "..itemName(prev["item"]).."</b> para roubar.",5000)
						return false
					...
					if vRP.tryGetInventoryItem(user_id,consultItem[2],1) then
						...
						for k,v in pairs(gangResult) do
							async(function()
								TriggerClientEvent("NotifyPush",v,{ code = "31", title = prev["name"], x = prev["coords"][1], y = prev["coords"][2], z = prev["coords"][3], time = "Recebido às "..os.date("%H:%M"), blipColor = 22 })
								vRPC.playSound(v,"Beep_Green","DLC_HEIST_HACKING_SNAKE_SOUNDS")
							end)
